...
class Tmp_mst_modul extends MX_Controller {
    ...
    public function form($id='')
    {
        ...
        if( $id != '' ){
            ...
            $this->breadcrumbs->push('Edit '.strtolower($this->title).'', 'Tmp_mst_modul/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
            /*get value by id*/
            ...
            $this->breadcrumbs->push('Add '.strtolower($this->title).'', 'Tmp_mst_modul/'.strtolower(get_class($this)).'/form');
    ...
    public function show($id)
    {
        ...
        $this->breadcrumbs->push('View '.strtolower($this->title).'', 'Tmp_mst_modul/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
    ...
    public function process()
    {
        ...
        $val->set_rules('title', 'Modul Title', 'trim|required');
        ...
        $val->set_rules('is_new_tab', 'Open In New Tab?', 'trim|required');
        if($_POST['is_new_tab'] == 'Y'){
            $val->set_rules('link_on_new_tab', 'Link On New Tab', 'trim|required');
        }
        ...
        {                       
            ...
            $dataexc = array(
                ...
                'is_new_tab' => $this->regex->_genRegex($val->set_value('is_new_tab'),'RGXAZ'),
                'is_active' => $this->regex->_genRegex($val->set_value('is_active'),'RGXAZ'),
                'is_deleted' => $this->regex->_genRegex('N','RGXAZ'),
            );
            if ($val->set_value('is_new_tab') =='Y') {
                $dataexc['link_on_new_tab'] = $this->regex->_genRegex($val->set_value('link_on_new_tab'),'RGXQSL');
            }else{
                $dataexc['link_on_new_tab'] = NULL;
            }
            if($id==0){
                $dataexc['created_date'] = date('Y-m-d H:i:s');
                $dataexc['created_by'] = json_encode(array('user_id' =>$this->regex->_genRegex($this->session->userdata('user')->user_id,'RGXINT'), 'fullname' => $this->regex->_genRegex($this->session->userdata('user')->fullname,'RGXQSL')));
                /*save post data*/
                $newId = $this->Tmp_mst_modul->save($dataexc);
                /*save logs*/
                $this->logs->save('tmp_mst_modul', $newId, 'insert new record on '.$this->title.' module', json_encode($dataexc),'modul_id');
            }else{
                $dataexc['updated_date'] = date('Y-m-d H:i:s');
                $dataexc['updated_by'] = json_encode(array('user_id' =>$this->regex->_genRegex($this->session->userdata('user')->user_id,'RGXINT'), 'fullname' => $this->regex->_genRegex($this->session->userdata('user')->fullname,'RGXQSL')));
                /*update record*/
                $this->Tmp_mst_modul->update(array('modul_id' => $id), $dataexc);
                $newId = $id;
                $this->logs->save('tmp_mst_modul', $newId, 'update record on '.$this->title.' module', json_encode($dataexc), 'modul_id');
            }
    ...
    public function delete()
    {
        ...
        if($id!=null){
            if($this->Tmp_mst_modul->delete_by_id($toArray)){
                $this->logs->save('tmp_mst_modul', $id, 'delete record', '', 'modul_id');
