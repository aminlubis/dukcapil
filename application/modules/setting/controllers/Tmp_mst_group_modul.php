...
class Tmp_mst_group_modul extends MX_Controller {
    ...
    public function form($id='')
    {
        ...
        if( $id != '' ){
            ...
            $this->breadcrumbs->push('Edit '.strtolower($this->title).'', 'Tmp_mst_group_modul/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
            /*get value by id*/
            ...
            $this->breadcrumbs->push('Add '.strtolower($this->title).'', 'Tmp_mst_group_modul/'.strtolower(get_class($this)).'/form');
    ...
    public function show($id)
    {
        ...
        $this->breadcrumbs->push('View '.strtolower($this->title).'', 'Tmp_mst_group_modul/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
    ...
    public function process()
    {
        ...
        {                       
            ...
            if($id==0){
                ...
                $newId = $this->Tmp_mst_group_modul->save($dataexc);
                /*save logs*/
                $this->logs->save('tmp_mst_group_modul', $newId, 'insert new record on '.$this->title.' module', json_encode($dataexc),'group_modul_id');
            }else{
                $dataexc['updated_date'] = date('Y-m-d H:i:s');
                $dataexc['updated_by'] = json_encode(array('user_id' =>$this->regex->_genRegex($this->session->userdata('user')->user_id,'RGXINT'), 'fullname' => $this->regex->_genRegex($this->session->userdata('user')->fullname,'RGXQSL')));
                /*update record*/
                $this->Tmp_mst_group_modul->update(array('group_modul_id' => $id), $dataexc);
                $newId = $id;
                /*save logs*/
                $this->logs->save('tmp_mst_group_modul', $newId, 'update record on '.$this->title.' module', json_encode($dataexc),'group_modul_id');
            }
    ...
    public function delete()
    {
        ...
        if($id!=null){
            if($this->Tmp_mst_group_modul->delete_by_id($toArray)){
                $this->logs->save('tmp_mst_group_modul', $id, 'delete record', '', 'group_modul_id');
