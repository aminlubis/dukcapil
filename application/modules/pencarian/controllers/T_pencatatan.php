...
class T_pencatatan extends MX_Controller {
    ...
    public function form($id='')
    {
        ...
        if( $id != '' ){
            ...
            $this->breadcrumbs->push('Edit '.strtolower($this->title).'', 'pencarian/T_pencatatan/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
            /*get value by id*/
            $data['value'] = $this->T_pencatatan->get_by_id($id);
            /*initialize flag for form*/
            $data['flag'] = "update";
        }else{
            /*breadcrumbs for create or add row*/
            $this->breadcrumbs->push('Add '.strtolower($this->title).'', 'pencarian/T_pencatatan/'.strtolower(get_class($this)).'/form');
        ...
        /*load form view*/
        $this->load->view('T_pencatatan/form', $data);
    }
    ...
    public function form_data_awal($id='')
    {
        ...
        $data['breadcrumbs'] = $this->breadcrumbs->show();
        /*load form view*/
        $this->load->view('T_pencatatan/form_data_awal', $data);
    }
        ...
        foreach ($list as $row_list) {
            ...
            $row[] = '<div class="center"><div class="btn-group">
                        ...
                        <ul class="dropdown-menu dropdown-inverse">
                            '.$this->authuser->show_button_dropdown('pencarian/T_pencatatan', array('R','U','D') ,$row_list->id_bencana).'   
    ...
    public function process()
    {
        ...
        {                       
            ...
            if(isset($_FILES['foto_default'] ['name']) AND $_FILES['foto_default'] ['name'] != ''){
                ...
                if( $id != 0 ){
                    $res_dt = $this->T_pencatatan->get_by_id($id);
            ...
            if($id==0){
                ...
                /*save post data*/
                $this->T_pencatatan->save($dataexc);
                $newId = $this->db->insert_id();
            }else{
                $dataexc['updated_date'] = date('Y-m-d H:i:s');
                $dataexc['updated_by'] = json_encode(array('user_id' =>$this->regex->_genRegex($this->session->userdata('user')->user_id,'RGXINT'), 'fullname' => $this->regex->_genRegex($this->session->userdata('user')->fullname,'RGXQSL')));
                /*update record*/
                $this->T_pencatatan->update(array('id_bencana' => $id), $dataexc);
                $newId = $id;
            }
    ...
    public function delete()
    {
        ...
        $toArray = explode(',',$id);
        if($id!=null){
            if($this->T_pencatatan->delete_by_id($toArray)){
    ...
    public function set_data_final()
    {
        $id=$this->input->post('ID')?$this->input->post('ID',TRUE):null;
        if($id!=null){
            if( $this->T_pencatatan->update(array('id_bencana' => $id), array('status_data' => 'final') ) ){
    ...
    private function sentToTopic($id)
    {
        ...
        if (is_null($data->notif_sent_at)) {
            $this->T_pencatatan->update( array('id_bencana' => $id), array('notif_sent_at' => date('Y-m-d H:i:s'), 'channel' => 'private-1_'.$id.'' ) );
